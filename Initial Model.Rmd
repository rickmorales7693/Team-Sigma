...
```
Optimal Clusters Computation for Unfiltered Model
```{r}
fviz_nbclust(train_bin_nf, kmeans, method = "wss")
```
...
# ROC curve
rf_roc_2 <- roc(as.numeric(as.character(train_bin_2$ECIG_BINARY)), rf_probs_2)
...
legend(x = "bottomright", legend = c("Asthmatics (Binary)", "Asthmatics & Smokers (Binary)", "All Adults (Multi-class)", "All Adults (Binary)"), fill = c("blue", "forestgreen", "orange", "brown"))
```
```{r}
