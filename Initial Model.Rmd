...
# Imputation of Missing Values ECIG_BIN
...
methods(mice)
imp_bin <- mice(train_bin, method = "cart", m = 1, maxit = 1, seed = 10)
...
```{r}
ggplot(train_bin, aes(x = as.factor(ECIG_BINARY), fill = as.factor(ECIG_BINARY))) + 
    geom_bar() + 
    scale_x_discrete(labels = c('Never Used', 'Current or Former')) + 
    labs(title = "E-Cigarette Smoking Status (Collapsed Binary Variable)", x = "E-Cig Smoking Status")
```

Predictor and Target Setup
```{r}
...
# ROC curve
rf_roc <- roc(train_bin$ECIG_BINARY, rf_probs)
...
```{r}
PR <- pr.curve(scores.class0 = )
```
```{r}
predict_prob <- predict(randForMod, test_bin, type = "prob")
...
```{r}
ggplot(train, aes(x = as.factor(ECIG_COMP), fill = as.factor(ECIG_COMP))) + 
    geom_bar() + 
