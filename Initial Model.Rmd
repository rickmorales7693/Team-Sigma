...
Train-Test Split (asthmatics with history of smoking)
```{r}
# Binary Variable Creation
...
```
Asthmatics w/ history of smoking
```{r}
# Calculate imputed values with training data
methods(mice)
#seed = 10 for Asthmatics, seed = 11 without filter
imp_bin_2 <- mice(train_bin_2, method = "cart", m = 1, maxit = 1, seed = 13)
...
```
Asthmatics with Smoking History
```{r}
...
Predictor and Target setup asthmatics and smokers
```{r}
# Convert factor predictors to numeric if needed
...
Random Forest Model Asthmatics
```{r}
...
Random Forest asthmatics and smokers
```{r}
# Convert outcome to factor for classification
...
# Plot ROC
b <- plot(rf_roc_2, col = "forestgreen", lwd = 2, main = "Random Forest ROC Curve", add = TRUE, print.auc = TRUE)
abline(a = 0, b = 1, lty = 2, col = "red")
```


ROC AUC not filtered
```{r}
# Predicted probabilities
...
# Plot ROC
c <- plot(rf_roc_nf, col = "forestgreen", lwd = 2, main = "Random Forest ROC Curve")
abline(a = 0, b = 1, lty = 2, col = "red")
```
Random forest combine
```{r}
library(ggpubr)
combined <- ggarrange(a,
                      b,
                      c,
                      nrow = 2,
                      ncol = 1)
...
randForMod
```
Addressing Class Imbalance and Overfitting (asthma and smoker)
```{r}
# Rename the factor levels
...
```
Feature Importance (asthma and smoker)
```{r}
plot(randForMod_2)
...
Predicting Data (asathma and smoker)
```{r}
...
methods(mice)
#seed = 10 for Asthmatics, seed = 11 without filter
imp <- mice(train_lr, method = "cart", m = 1, maxit = 1, seed = 12)

train_lr <- complete(imp)
```
...
p4 <- ggplot(train_lr, aes(x = as.factor(ECIG_COMP), 
    ...
    labs(title = "E-Cigarette Smoking Status Among All Adults Surveyed Xin 2017", x = "E-Cig Smoking Status", y = "Surveyed Population %", fill = "E-Cig Smoking Status")
```
...
# Create predictor matrix
x <- data.matrix(train_lr[, predictors])
...
# Predict probabilities
predict_lr <- predict(randForMod, newdata = test_lr)
predict_prob_lr <- predict(randForMod, newdata = test_lr, type = "prob")
...
library(yardstick)
#df_lr <- dplyr::bind_cols(truth = y_train, prediction = predict_lr)
#roc_auc(df_lr, y_train, .pred_classname:.pred_classname, estimator = "hand_till")
```
```{r}
# ROC curve
rf_probs_lr <- predict(model, type = "prob")[, 2]  # probability of class "1"
...
# Plot ROC
d <- plot(rf_roc_lr, col = "forestgreen", lwd = 2, main = "Ordinal Logistic Regression ROC Curve")
abline(a = 0, b = 1, lty = 2, col = "red")
```
```{r}
library(gridExtra)
...
# Plot ROC
d <- plot(rf_roc_lr, col = "orange", lwd = 2, main = "Ordinal Logistic Regression ROC Curve", add = TRUE, print.auc = TRUE)
abline(a = 0, b = 1, lty = 2, col = "red")
...
# Plot ROC
c <- plot(rf_roc_nf, col = "brown", lwd = 2, main = "Random Forest ROC Curve", add = TRUE, print.auc = TRUE)
abline(a = 0, b = 1, lty = 2, col = "red")
```
