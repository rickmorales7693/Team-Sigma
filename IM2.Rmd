...
# Filter for valid ECIG_COMP, asthma history, and current asthma
...
# Create labeled cross-tabulation summary
table(brfss_2017_cleaned$ASTHMA_NOW_COMP, brfss_2017_cleaned$ASTHMA)

asthma_table <- brfss_2017_cleaned %>%
  mutate(
    ASTHMA_LABEL = case_when(
      ASTHMA == 1 ~ "ASTHMA == 1\n(Yes, ever told)",
      ASTHMA == 2 ~ "ASTHMA == 2\n(No, never told)",
      ASTHMA %in% c(7, 9) ~ "ASTHMA == 9\n(Don't know/refused/missing)"
    ),
    ASTHMA_NOW_LABEL = case_when(
      ASTHMA_NOW_COMP == 1 ~ "1 (Currently has asthma)",
      ASTHMA_NOW_COMP == 2 ~ "2 (Does NOT currently have asthma)",
      ASTHMA_NOW_COMP == 9 ~ "9 (Missing/invalid)"
    )
  ) %>%
  filter(!is.na(ASTHMA_LABEL), !is.na(ASTHMA_NOW_LABEL)) %>%
  count(ASTHMA_NOW_LABEL, ASTHMA_LABEL) %>%
  pivot_wider(names_from = ASTHMA_LABEL, values_from = n, values_fill = 0)
